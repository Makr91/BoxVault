...
class FileService {
  async upload(file, organization, name, version, provider, architecture, checksum, checksumType, onUploadProgress) {
    ...
    try {
        ...
        try {
          ...
          if (typeof fileSize === 'number') {
            ...
            if (sizeDiff > maxDiff) {
              ...
              throw new Error(`File size mismatch after assembly: Expected ${file.size} bytes but got ${fileSize} bytes`);
            }
      ...
      throw new Error(`Upload failed: Assembly timed out after ${Math.round((Date.now() - startTime) / 1000)} seconds`);
    } catch (error) {
