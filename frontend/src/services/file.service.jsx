...
class FileService {
  ...
  buildChunkHeaders(uploadOptions, currentChunk, totalChunks, chunkSize, fileName) {
  ...
  async pollForAssembly(uploadOptions, fileSize, onUploadProgress) {
    const { organization, name, version, provider, architecture } = uploadOptions;
    let attempts = 0;
    let delay = 1000;
    const maxDelay = 5000;
  ...
  async upload(file, uploadOptions, onUploadProgress) {
    const { organization, name, version, provider, architecture, checksum, checksumType } =
      uploadOptions;
    ...
    try {
      ...
      return await this.pollForAssembly(uploadOptions, file.size, onUploadProgress);
    } catch (error) {
