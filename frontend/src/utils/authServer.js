...
const fetchTrustedIssuers = async () => {
  const response = await fetch(`${window.location.origin}/api/auth/oidc/issuers`);
...
const isIssuerTrusted = (issuer, trustedIssuers) => {
  ...
  if (!isTrusted) {
    log.auth.warn('Issuer not in trusted whitelist', {
      issuer,
      normalizedIssuer,
      trustedIssuers: trustedIssuers.map(trustedIssuer => normalizeUrl(trustedIssuer.issuer)),
    });
  }
  return isTrusted;
};
