...
const Navbar = ({
    ...
    if (currentUser) {
      UserService.updatePreferences({ language: lng })
        .then(() => {
          ...
          if (stored) {
            localStorage.setItem('user', JSON.stringify({ ...stored, preferredLanguage: lng }));
          }
        })
        .catch(error => {
  ...
  const renderOrgIcon = () => {
    if (activeOrgGravatar) {
      return <img src={activeOrgGravatar} alt="" className="rounded-circle avatar-sm me-2" />;
    }
  ...
  const renderUserMenu = () => (
    <li className="nav-item dropdown user-menu">
      ...
      <ul className="dropdown-menu dropdown-menu-end" aria-labelledby="navbarDropdown">
        ...
        {hasNotificationsScope(userClaims) && <NotificationsItem authServerUrl={authServerUrl} />}
        {ticketUrl && (
          <li>
            <a href={ticketUrl} target="_blank" rel="noopener noreferrer" className="dropdown-item">
    ...
    <nav className="navbar navbar-expand-lg sticky-top shadow-sm bg-body-tertiary border-bottom">
      <div className="container-fluid">
        <Link to="/" className="navbar-brand p-0 d-flex align-items-center">
          <BrandLogo theme={theme} className="logo-cluster icon-with-margin-sm" />
          BoxVault
        </Link>
