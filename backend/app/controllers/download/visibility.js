 ...
 * organizations plus the unpublished ones they created.
 * @param {{userId: number, orgIds: number[]}|null} viewer - From resolveDownloadViewer
...
const downloadWhereFor = (viewer, organizationId) => {
  if (organizationId !== undefined) {
    return viewer && viewer.orgIds.includes(organizationId)
      ...
      : { organizationId, ...PUBLIC_DOWNLOAD };
  }
  if (!viewer) {
...
/**
 * Whether a viewer may read one download: public and published, or a member
 * of its organization when it is published or the viewer created it.
 * @param {{userId: number, orgIds: number[]}|null} viewer - From resolveDownloadViewer
...
/**
 * Whether a viewer is a member of an organization, the rule the download
 * counts are answered by: the number to a member, null to anyone else.
